...
public:
    ...
    POneEquation(const AMRParam& amrpp, const MLMGParam& mlmgpp,
        ...
        : amrpp_(amrpp), mlmgpp_(mlmgpp), geom_(geom), grids_(grids)
    {
        initializeEB();
    ...
    void solve()
    {
        ...
        for (int ilev = 0; ilev <= max_level; ++ilev)
        {
            ...
            // setScalars (Real A, Real B)
            // for P1 A = 1.0, B = 1.0/3.0
            mleb.setScalars(scalars[0], scalars[1]);

            // setACoeffs (int amrlev, MultiFab alpha)
            // for P1 alpha = 1.0.
            mleb.setACoeffs(0, acoef_[ilev]);
            // setBCoeffs (int amrlev, MultiFab beta)
            // for P1 beta = 1/k.
            ...
            // solve (a_sol, a_rhs, a_tol_rel, a_tol_abs).
