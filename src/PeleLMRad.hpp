            ...
            [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept
            {
                ...
                if (j >= dlo.y && j <= dhi.y && k >= dlo.z && k <= dhi.z)
                {
                    ...
                    if (i > dhi.x)
                    {
                        robin_cell = true;
                        betafab(i, j, k)
                            = 1.0 / std::max(1.0, kappa(dhi.x, jj, kk) * 100);
                    }
                    if (i < dlo.x)
                    {
                        robin_cell = true;
                        betafab(i, j, k)
                            = 1.0 / std::max(1.0, kappa(dlo.x, jj, kk) * 100);
                    }
                }
                else if (i >= dlo.x && i <= dhi.x && k >= dlo.z && k <= dhi.z)
                {
                    ...
                    if (j > dhi.y)
                    {
                        robin_cell = true;
                        betafab(i, j, k)
                            = 1.0 / std::max(1.0, kappa(ii, dhi.y, kk) * 100);
                    }
                    if (j < dlo.y)
                    {
                        robin_cell = true;
                        betafab(i, j, k)
                            = 1.0 / std::max(1.0, kappa(ii, dlo.y, kk) * 100);
                    }
                }
                else if (i >= dlo.x && i <= dhi.x && j >= dlo.y && j <= dhi.y)
                {
                    ...
                    if (k > dhi.z)
                    {
                        robin_cell = true;
                        betafab(i, j, k)
                            = 1.0 / std::max(1.0, kappa(ii, jj, dhi.z) * 100);
                    }
                    if (k < dlo.z)
                    {
                        robin_cell = true;
                        betafab(i, j, k)
                            = 1.0 / std::max(1.0, kappa(ii, jj, dlo.z) * 100);
                    }
                }
